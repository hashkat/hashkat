...
class MainWindow(object):
    def __init__(self, root):
        ...
        Executor.bind('test_output_update', self.on_executorOutputUpdate)
    ...
    def _setup_left_frame(self):
        ...
        self.tree_notebook = Notebook(self.content, padding=(0, 5, 0, 5), width=500)
    ...
    def _add_test_module(self, parentNode, testModule):
        ...
        for subModuleName, subModule in sorted(testModule.items()):
            if isinstance(subModule, TestModule):
                ...
                #    testMethod_node = self.all_tests_tree.insert(
    ...
    def on_testCaseSelected(self, event):
        ...
        for part in parts:
            testMethod = testMethod[part]
        for _, testMethodPrime in testMethod.items():
            testMethod = testMethodPrime
    ...
    def on_testMethodSelected(self, event):
        ...
        if len(event.widget.selection()) == 1:
            ...
            self.description.delete('1.0', END)
            if not hasattr(testMethod, 'description'):
                for _, testMethodPrime in testMethod.items():
                    testMethod = testMethodPrime
            self.description.insert('1.0', testMethod.description)
            config = STATUS.get(testMethod.status, STATUS_DEFAULT)
    ...
    def on_nodeStatusUpdate(self, node):
        ...
        for path in node.parent.path,:
            self.all_tests_tree.item(path, tags=['TestCase', STATUS[node.status]['tag']])
        ...
        if node.status in TestMethod.FAILING_STATES:
            ...
            for path in node.parent.path,:
                self.problem_tests_tree.item(path, tags=['TestMethod', STATUS[node.status]['tag']])
        else:
    ...
    def on_executorOutputUpdate(self, event, output):
        self._show_test_output(output)
